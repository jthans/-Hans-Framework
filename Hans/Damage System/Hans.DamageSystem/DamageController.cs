...
namespace Hans.DamageSystem
{
        ...
        where T : DamageUnit
    {
        ...
        public DamageUnit ApplyDamage(string entityId, DamageUnit damageAmt)
        {
            ...
            this.log.LogMessage($"Normalized Damage: { string.Join(" ", normalizedDamage.Select(x => $"{ x.Key }: { x.Value } "))}");
            ...
            this.log.LogMessage($"Layer-Effect Damage: { string.Join(" ", normalizedDamage.Select(x => $"{ x.Key }: { x.Value } "))}");

            this.CalculateDamageTypeEffects(ref normalizedDamage);
            this.log.LogMessage($"Damage-Type Damage: { string.Join(" ", normalizedDamage.Select(x => $"{ x.Key }: { x.Value } "))}");
